...
export interface IBlockTemplate {
  ...
  vbavailable: Record<string, number>; // (numeric) identifies the bit number as indicating acceptance and readiness for the named softfork rule
  ...
  coinbaseaux: Record<string, string>; //'hex',              // (string) values must be in the coinbase (keys may be ignored)
  ...
  signet_challenge?: string; // (string, optional) signet challenge scriptPubKey hex
  capabilities: string[];
}
