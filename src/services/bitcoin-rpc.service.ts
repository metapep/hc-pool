...
export class BitcoinRpcService implements OnModuleInit {
  ...
  public async pollMiningInfo() {
    ...
    if (
      miningInfo != null &&
      (!this.hasPublishedMiningInfo || miningInfo.blocks > this.blockHeight)
    ) {
  ...
  private async loadBlockTemplate(blockHeight: number) {
    ...
    while (blockTemplate == null) {
      const templateRequest: any = {
        ...
        // HashCash signet-enabled nodes require explicit segwit+signet rule support in GBT requests.
        rules: ['segwit', 'signet'],
      };
  ...
  public async getMediumFeeSatVb(): Promise<number> {
    ...
    try {
      ...
      console.warn(
        `Error estimatesmartfee positional-call: ${e?.message ?? e}`,
      );
    }
